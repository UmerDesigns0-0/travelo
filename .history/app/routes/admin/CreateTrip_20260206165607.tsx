          ...
          <FieldSet>
            ...
            <SelectItems
              onChange={(value: string) =>
                setFormData((prevData) => ({ ...prevData, groupType: value }))
              }
            />
            <Maps selectedCountry={selectedCountry} />
            {error && (
              <p className="text-red-500 text-sm mt-2 text-center transition duration-200 animate-[pulse_1s_ease-in-out]">
                {error}
              </p>
            )}
            <button
              className={cn(`flex items-center justify-center gap-2 transition-colors duration-300 ease-in-out bg-blue-500 hover:bg-blue-600 text-sm w-full md:w-sm mx-auto py-3 rounded-md text-white cursor-pointer`, loading && "cursor-not-allowed opacity-70")}
              type="submit"
              disabled={loading}
            >
              {" "}
