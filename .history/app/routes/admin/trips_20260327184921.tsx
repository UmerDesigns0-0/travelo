...
const Trips = () => {
  ...
  useEffect(() => {
    if (currentUser && !sort) {
      getExistingUser(currentUser.$id).then(existingUser => {
        if (existingUser?.status !== "admin") {
          navigate(`/trips?sort=my_trips&userId=${currentUser.$id}`, { replace: true });
        }
      });
    }
  }, [currentUser, sort, navigate]);
  ...
  return (
    <main className="all-users wrapper">
      ...
      <section>
        ...
        <div className={cn(`trip-grid-tripsPage`, totalPages <= 1 && "mb-8")}>
          {allTrips.map(
            ({
              id,
              name,
