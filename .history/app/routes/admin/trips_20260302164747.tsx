...
const Trips = () => {
  ...
  console.log("Filter changed to:", newSort); // Debug log

  // Handle "My Trips" filter
  if (newSort === "my_trips") {
    setCurrentSort("latest"); // Default to latest for my trips
    navigate(
      `/trips?limit=${limit}&page=1` +
        `${searchTerm ? `&search=${encodeURIComponent(searchTerm)}` : ""}` +
        `&sort=latest` +
        `&filter=mine` // This is critical!
    );
    return;
  }

  // Handle "All Trips" - remove filter
  if (newSort === "all_trips") {
    ...
    navigate(
      `/trips?limit=${limit}&page=1` +
        `${searchTerm ? `&search=${encodeURIComponent(searchTerm)}` : ""}` +
        `&sort=latest`
      // No filter parameter = show all trips
    );
    return;
  }

  // Handle sort changes (latest, oldest)
  setCurrentSort(newSort);
  
  // Check current URL for filter parameter
  ...
  navigate(
    `/trips?limit=${limit}&page=1` +
      `${searchTerm ? `&search=${encodeURIComponent(searchTerm)}` : ""}` +
      `&sort=${newSort}` +
      `${currentFilter === "mine" ? `&filter=mine` : ""}`
  );
};
