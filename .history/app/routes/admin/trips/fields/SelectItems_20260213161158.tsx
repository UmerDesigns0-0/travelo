...
const SelectItems = ({ onChange, error, formData }: SelectItemsProps) => {
  ...
  const fieldErrors: Record<string, string> = {
    groupType: "Please select a group type.",
    travelStyle: "Please select a travel style.",
      ...
      {selectItems.map((key) => (
        <Field key={key}>
          ...
          <FieldDescription className="text-xs">
            {error && !formData[key as keyof typeof formData] ? (
              <span className="text-red-500">{fieldErrors[key]}</span>
            ) : (
