...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const sort =
    (url.searchParams.get("sort") as "" | "latest" | "oldest") || undefined;
  const filter = url.searchParams.get("filter") || "all";

  // when the user wants to see "mine" we need to resolve the current
  // user's ID on the server; we don't trust a client-provided value.
  ...
  if (filter === "mine") {
    try {
      ...
      // if there is no session the query will just return no results
      userId = undefined;
    }
  }
...
const Trips = () => {
  const { allTrips, total, limit, page, search, sort, userId } =
    useLoaderData() as {
      allTrips: Trip[];
      total: number;
      limit: number;
  ...
  // we no longer need to fetch the user ID client-side; it is handled
  // by the loader and returned in `userId` above.
  // const [usersId, setUsersId] = useState<string | null>(null);

  const navigate = useNavigate();

  // client-side lookup removed; see loader comment above
  // useEffect(() => {
  //   account.get()
  //     .then(u => setUsersId(u.$id))
  //     .catch(() => {});
  // }, []);
  ...
  const heading = (() => {
    if (filter === "mine" && sort === "latest") return "My Newest Trips";
    if (filter === "mine" && sort === "oldest") return "My Oldest Trips";
    if (filter === "mine") return "My Trips";
    if (filter === "all_trips") return "All Trips";
