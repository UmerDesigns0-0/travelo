...
const Trips = () => {
  ...
  const handleSearch = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    navigate(`/trips?limit=${limit}&page=1&search=${encodeURIComponent(search || "")}&sort=${sort}&userId=${userId}`);
  }

  const handleFilterChange = (newSort: string) => {
    navigate(`/trips?limit=${limit}&page=1&search=${encodeURIComponent(search || "")}&sort=${newSort}&userId=${userId}`);
  }
