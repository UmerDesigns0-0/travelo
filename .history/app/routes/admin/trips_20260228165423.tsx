...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const sortParam = (url.searchParams.get("sort") as "" | "latest" | "oldest") || "latest";
  const userId = url.searchParams.get("userId") || undefined;

  const offset = (page - 1) * limit;

  // Map "latest" to "newest" for the API
  const sort = sortParam === "latest" ? "newest" : sortParam;
...
const Trips = () => {
  const { allTrips, total, limit, page, search, sort, userId } = useLoaderData() as {
    allTrips: Trip[];
    total: number;
    limit: number;
    page: number;
  ...
  const [currentSort, setCurrentSort] = useState<string>(sort || "latest");
