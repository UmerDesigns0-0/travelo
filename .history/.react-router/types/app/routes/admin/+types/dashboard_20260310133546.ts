...
export const UsersAndTripsStats = async (): Promise<DashboardStats> => {
  const d = new Date();
  ...
  const filterByDate = ({items, key, start, end}: filterByDate) => {
    return items.filter(item => item[key] >= start && item[key] <= end).length;
  };
