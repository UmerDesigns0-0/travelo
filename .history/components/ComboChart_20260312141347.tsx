...
const ComboChart = ({
  ...
  const ratio = value / maxValue; // 0 to 1

  // Pick segment count based on ratio
  const segments =
    ratio > 0.75 ? 4 : ratio > 0.5 ? 3 : ratio > 0.25 ? 2 : 1;

  const colors = ["#bfdbfe", "#93c5fd", "#60a5fa", "#3b82f6"]; // light → dark
  const segHeight = height / segments;

  return (
    <g>
      {Array.from({ length: segments }).map((_, i) => {
        const segY = y + i * segHeight;
        const isTop = i === 0;
        const isBottom = i === segments - 1;
        const r = 8;
        ...
        return (
          <rect
            ...
            height={segHeight + 1}
            fill={colors[4 - segments + i]} // always end on darkest
            rx={isTop ? r : 0}
            ry={isTop ? r : 0}
          />
        );
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              <defs>
                ...
                >
                  {/* near line */}
                  <stop offset="0%" stopColor="#a9d0fc" stopOpacity={1} />

                  {/* middle */}
                  <stop offset="50%" stopColor="#bcd8f7" stopOpacity={0.6} />

                  {/* bottom */}
                  <stop offset="100%" stopColor="#dce8f7" stopOpacity={0.2} />
                </linearGradient>
              ...
              <Bar
                ...
                radius={8}
                shape={(props: any) => <DynamicBar {...props} data={formatedData} />}
              />
