...
const ComboboxComponent = <T extends BaseItem>({
  ...
  return (
    <Combobox
      items={data}
      onValueChange={(value) => {
        if (!value) return; // Guard against null/empty

        const stringValue =
          typeof value === "object"
            ? (value as any).value // If library returns object
            : (value as string); // If library returns string

        const item = data.find((d) => d.value === stringValue);
        onValueChange?.(stringValue, item?.value ? item : undefined);
      }}
    >
      <ComboboxTrigger
        render={
