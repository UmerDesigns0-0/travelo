      ...
      cell: ({ getValue }) => {
        ...
        return (
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 rounded-full truncate  flex items-center justify-center text-sm font-medium text-white">
              <img className="h-5 w-5 bg-green-300 rounded-full" src={trip?.url} alt="" />
            </div>
            <span>{trip?.name}</span>
          </div>
        );
