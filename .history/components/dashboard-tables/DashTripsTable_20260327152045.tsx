        ...
        return (
          <Link to="/trips/:$id" className="flex items-center gap-2">
            <div className="h-8 w-8 rounded-full truncate bg-green-300 flex items-center justify-center text-sm font-medium text-white">
              <img className="h-8 w-8 rounded-full hover:scale-110 transition duration-300 ease-in-out" src={trip?.url} alt="" />
            </div>
            <span title={trip?.name} className="w-50 line-clamp-1 hover:underline underline-offset-4">
              {trip?.name}
            </span>
          </Link>
        );
