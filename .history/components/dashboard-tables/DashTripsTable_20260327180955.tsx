  ...
  const columns: ColumnDef<Trips>[] = [
    {
      ...
      cell: ({ getValue, row }) => {
        ...
        return (
          <Link to={`/trips/${tripId}`} className="flex items-center gap-2 group">
            <div className="h-8 w-8 rounded-full overflow-hidden bg-green-300 flex items-center justify-center text-sm font-medium text-white">
              {trip?.url && (trip?.url !== null || trip?.url !== undefined) && (
                <img
                  className="h-full w-full object-contain group-hover:scale-110 transition duration-300 ease-in-out"
                  src={trip?.url}
                  alt=""
                />
              )}
